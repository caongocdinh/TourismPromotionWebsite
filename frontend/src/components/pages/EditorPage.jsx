import { useState, useEffect, useCallback } from 'react';
  ...
  const EditorPage = () => {
    ...
    const { user, token } = useAuth(); // Lấy user và token từ useAuth
    ...
    const handlePublish = useCallback(async () => {
      if (!editor || !user) return; // Kiểm tra user tồn tại
      try {
        ...
        await axios.post('http://localhost:5000/api/posts/add', {
          title,
          content,
          user_id: user.id, // Sử dụng user.id từ auth
          touristPlaces,
          ...
          headers: {
            ...
            'Content-Type': 'application/json', // Đảm bảo đúng cho JSON
          },
        });
